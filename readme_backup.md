...
Uses Normal likelihood but computes **robust (sandwich) standard errors** that are valid even when the true distribution is non-Normal. Invoke by passing `method='qmle'`.

```python
from volkit import GARCH, Normal, StudentT, GJRGARCH

# QMLE with Normal density
spec = GARCH(1, 1) + Normal()
...
# QMLE with Student-t (two-step: Normal LL for GARCH, then fit nu)
spec = GARCH(1, 1) + StudentT()
result = spec.fit(data, method='qmle')

# GJR-GARCH is also supported
...
```python
result = spec.fit(
    data,                      # 1D array, pandas Series, or DataFrame
    method="mle",              # "mle" (default) or "qmle"
    solver="trust",            # Optimization method
...
# QMLE — robust standard errors
result = spec.fit(data, method="qmle")
result.std_errors_robust     # Sandwich (robust) SEs
```
