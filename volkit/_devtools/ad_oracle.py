...
def garch_logspace_value_grad_hess(
    ...
    def objective(z_theta):
        ...
        omega = jnp.exp(jnp.clip(z_garch[0], -700.0, 700.0))
        ...
        if dist == "studentt":
            nu = 2.0 + jnp.log1p(jnp.exp(z_theta[-1]))
            inv_nu_m2 = 1.0 / (nu - 2.0)
            constant = n * (
                jsp.special.gammaln(0.5 * (nu + 1.0))
                - jsp.special.gammaln(0.5 * nu)
        ...
        if dist == "skewt":
            ...
            nu = 2.0 + jnp.log1p(jnp.exp(z_theta[-2]))
...
def gjr_garch_logspace_value_grad_hess(
    ...
    def objective(z_theta):
        ...
        lse = jsp.special.logsumexp(z_joint)
        omega = jnp.exp(jnp.clip(z_gjr[0], -700.0, 700.0))
...
def arma_garch_logspace_value_grad_hess(
    ...
    def objective(z_theta):
        ...
        lam = None
        if dist == "studentt":
            nu = 2.0 + jnp.log1p(jnp.exp(z_theta[n_mean + n_vol]))
        elif dist == "skewt":
            nu = 2.0 + jnp.log1p(jnp.exp(z_theta[n_mean + n_vol]))
