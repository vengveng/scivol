# volkit/_kernels/arma_garch_normal.py
"""
ARMA(p,q) + GARCH(P,Q) + Normal kernel.

Currently implements specialized ARMA(1,1)+GARCH(1,1) using C extensions.
"""
from __future__ import annotations
import re
import time
import numpy as np
from numpy.typing import NDArray
from typing import Dict, Tuple

from .. import _core
from ..components.mean import ARMA
from ..components.vol import GARCH
from ..components.density import Normal
from ..spec.composite import CompositeSpec
from ..result import EstimationResult
from .routine import Routine

_CACHE: Dict[Tuple[int, int, int, int], Routine] = {}

# Regex to match ARMA(p,q)+GARCH(P,Q)+Normal
_RE_UID = re.compile(r"ARMA\((\d+),(\d+)\)\+GARCH\((\d+),(\d+)\)\+Normal$")


def _as_cptr(arr: NDArray[np.float64]) -> int:
    """Convert numpy array to C pointer (as integer address)."""
    return np.ascontiguousarray(arr, dtype=np.float64).ctypes.data


def _build(p_ar: int, q_ma: int, P_arch: int, Q_garch: int) -> Routine:
    """Build routine for ARMA(p,q)+GARCH(P,Q)+Normal."""
    uid = f"ARMA({p_ar},{q_ma})+GARCH({P_arch},{Q_garch})+Normal"
    
    mean = ARMA(p_ar, q_ma)
    vol = GARCH(P_arch, Q_garch)
    dens = Normal()
    spec = CompositeSpec(mean, vol, dens)
    
    n_mean = 1 + p_ar + q_ma  # c + AR + MA params
    n_vol = 1 + P_arch + Q_garch  # omega + alpha + beta
    n_params = n_mean + n_vol
    
    # Check if specialized C function exists
    if p_ar == 1 and q_ma == 1 and P_arch == 1 and Q_garch == 1:
        use_specialized = True
    else:
        use_specialized = False
    
    def fit(y: NDArray[np.float64], solver: str = "slsqp", log_mode: bool = False, verbose: bool = False, **_) -> EstimationResult:
        from scipy.optimize import minimize, LinearConstraint
        
        t_start = time.perf_counter()
        
        y = np.ascontiguousarray(y, dtype=np.float64)
        n = len(y)
        
        # Pre-allocate working arrays
        resid = np.zeros(n, dtype=np.float64)
        sigma2 = np.zeros(n, dtype=np.float64)
        
        # Initial variance = mean(y^2)
        h0 = np.mean(y ** 2)
        
        # Pointers
        y_ptr = _as_cptr(y)
        resid_ptr = _as_cptr(resid)
        sigma2_ptr = _as_cptr(sigma2)
        
        if use_specialized:
            # ARMA(1,1)+GARCH(1,1) specialized path
            # Params: [c, phi, theta, omega, alpha, beta]
            
            def objective(params: NDArray[np.float64]) -> float:
                nll = _core._arma_garch_nll_11_normal(
                    _as_cptr(params),
                    y_ptr,
                    resid_ptr,
                    sigma2_ptr,
                    h0,
                    n
                )
                return nll / n
            
            # Gradient available for Normal
            grad_vec = np.zeros(6, dtype=np.float64)
            grad_ptr = _as_cptr(grad_vec)
            
            def gradient(params: NDArray[np.float64]) -> NDArray[np.float64]:
                _core._arma_garch_nll_grad_11_normal(
                    _as_cptr(params),
                    y_ptr,
                    resid_ptr,
                    sigma2_ptr,
                    grad_ptr,
                    h0,
                    n
                )
                return grad_vec.copy() / n
            
            # Default start
            start = np.array([
                np.mean(y),  # c
                0.0,         # phi
                0.0,         # theta
                h0 * 0.1,    # omega
                0.05,        # alpha
                0.90,        # beta
            ], dtype=np.float64)
            
            # Bounds
            bounds = [
                (-1.0, 1.0),    # c
                (-0.99, 0.99),  # phi
                (-0.99, 0.99),  # theta
                (1e-10, None),  # omega
                (1e-10, 0.999), # alpha
                (1e-10, 0.999), # beta
            ]
            
            # Stationarity constraint: alpha + beta < 1
            A = np.array([[0, 0, 0, 0, 1, 1]])
            lc = LinearConstraint(A, lb=0.0, ub=0.9999)
            
        else:
            # General ARMA(p,q)+GARCH(P,Q) path
            # Params: [c, phi_1..phi_p, theta_1..theta_q, omega, alpha_1..alpha_P, beta_1..beta_Q]
            
            # Pre-allocate arrays for general case
            e0 = np.zeros(max(q_ma, 1), dtype=np.float64)
            h0_arr = np.full(max(Q_garch, 1), h0, dtype=np.float64)
            
            e0_ptr = _as_cptr(e0)
            h0_ptr = _as_cptr(h0_arr)
            
            def objective(params: NDArray[np.float64]) -> float:
                nll = _core._arma_garch_nll_pq_normal(
                    _as_cptr(params),
                    y_ptr,
                    resid_ptr,
                    sigma2_ptr,
                    e0_ptr,
                    h0_ptr,
                    n,
                    p_ar,
                    q_ma,
                    P_arch,
                    Q_garch
                )
                return nll / n
            
            gradient = None  # No gradient for general case yet
            
            # Default start
            start = np.concatenate([
                [np.mean(y)],                     # c
                [0.0] * p_ar,                     # phi
                [0.0] * q_ma,                     # theta
                [h0 * 0.1],                       # omega
                [0.05 / P_arch] * P_arch,         # alpha
                [0.90 / Q_garch] * Q_garch,       # beta
            ])
            
            # Bounds
            bounds = (
                [(-1.0, 1.0)] +                   # c
                [(-0.99, 0.99)] * p_ar +          # phi
                [(-0.99, 0.99)] * q_ma +          # theta
                [(1e-10, None)] +                 # omega
                [(1e-10, 0.999)] * P_arch +       # alpha
                [(1e-10, 0.999)] * Q_garch        # beta
            )
            
            # Stationarity constraint
            A = np.zeros((1, n_params))
            A[0, n_mean:n_mean + P_arch] = 1  # alpha
            A[0, n_mean + P_arch:] = 1        # beta
            lc = LinearConstraint(A, lb=0.0, ub=0.9999)
        
        # Optimize
        if solver.lower() == "nelder-mead":
            res = minimize(
                objective,
                start,
                method="Nelder-Mead",
                bounds=bounds,
                options={"maxfev": 50000, "disp": verbose, "fatol": 1e-10}
            )
        elif solver.lower() == "slsqp":
            if gradient is not None:
                res = minimize(
                    objective,
                    start,
                    method="SLSQP",
                    jac=gradient,
                    bounds=bounds,
                    constraints=lc,
                    options={"disp": verbose, "maxiter": 5000, "ftol": 1e-12}
                )
            else:
                res = minimize(
                    objective,
                    start,
                    method="SLSQP",
                    bounds=bounds,
                    constraints=lc,
                    options={"disp": verbose, "maxiter": 5000, "ftol": 1e-12}
                )
        elif solver.lower() in ["trust", "trust-constr"]:
            if gradient is not None:
                res = minimize(
                    objective,
                    start,
                    method="trust-constr",
                    jac=gradient,
                    bounds=bounds,
                    constraints=lc,
                    options={"disp": verbose, "maxiter": 5000}
                )
            else:
                res = minimize(
                    objective,
                    start,
                    method="trust-constr",
                    bounds=bounds,
                    constraints=lc,
                    options={"disp": verbose, "maxiter": 5000}
                )
        else:
            raise ValueError(f"Unknown solver: {solver}")
        
        t_elapsed = time.perf_counter() - t_start
        
        # Final evaluation to get sigma2
        _ = objective(res.x)
        
        # Create a modified optimization result with un-scaled objective
        # (EstimationResult expects -loglik as fun)
        class ScaledResult:
            def __init__(self, opt_res, n):
                self.x = opt_res.x
                self.fun = opt_res.fun * n  # Undo per-observation scaling
                self.success = opt_res.success
                self.nit = getattr(opt_res, 'nit', getattr(opt_res, 'nfev', 0))
                self.message = getattr(opt_res, 'message', '')
        
        scaled_res = ScaledResult(res, n)
        
        return EstimationResult(
            spec=spec,
            optimization_result=scaled_res,
            data=y,
            sigma2=sigma2.copy(),
            time_elapsed=t_elapsed,
        )
    
    return Routine(
        uid=uid,
        fit=fit,
        n_params=n_params,
        start=lambda y: np.concatenate([
            [np.mean(y)],
            [0.0] * p_ar,
            [0.0] * q_ma,
            [np.mean(y**2) * 0.1],
            [0.05 / P_arch] * P_arch,
            [0.90 / Q_garch] * Q_garch,
        ]),
        bounds=lambda: (
            [(-1.0, 1.0)] +
            [(-0.99, 0.99)] * p_ar +
            [(-0.99, 0.99)] * q_ma +
            [(1e-10, None)] +
            [(1e-10, 0.999)] * P_arch +
            [(1e-10, 0.999)] * Q_garch
        ),
    )


def get_routine(uid: str) -> Routine:
    """Public entry point for kernel registry."""
    m = _RE_UID.match(uid)
    if not m:
        raise RuntimeError(f"arma_garch_normal cannot handle '{uid}'")
    
    p_ar, q_ma, P_arch, Q_garch = map(int, m.groups())
    key = (p_ar, q_ma, P_arch, Q_garch)
    
    if key not in _CACHE:
        _CACHE[key] = _build(*key)
    
    return _CACHE[key]
