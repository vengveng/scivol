...
def _build(p_ar: int, q_ma: int, P_arch: int, Q_garch: int) -> Routine:
    ...
    def fit(y: NDArray[np.float64], solver: str = "slsqp", log_mode: bool = False, verbose: bool = False, **_) -> EstimationResult:
        ...
        from .transforms import (
            pack_arma_garch_normal, unpack_arma_garch_normal, jacobian_arma_garch_normal
        )
        ...
        if not log_mode:
            ...
            def pack(z: NDArray[np.float64]) -> NDArray[np.float64]:
                return pack_arma_garch_normal(z, p_ar, q_ma, P_arch, Q_garch)
            
            def unpack(theta: NDArray[np.float64]) -> NDArray[np.float64]:
                return unpack_arma_garch_normal(theta, p_ar, q_ma, P_arch, Q_garch)
