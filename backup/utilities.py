...
def wait_years(p, freq, daily_per_year=252, weekly_per_year=52):
    """
    Median waiting time (years) until observing >=1 event with prob 50%,
    given per-period event probability p (tail prob).
    """
    ...
    n50 = np.log(0.5) / np.log1p(-p)  # periods
    per_year = daily_per_year if freq == "d" else weekly_per_year
    return n50 / per_year
...
# --- helpers: "standardized" Student-t CDF used by likelihood (variance = 1) ---
...
def dgt_with_lb_moments(
    ...
    *,
    ...
    return {
        ...
        "u": u,          # pd.Series
        "dgt": dgt,      # dict
        "lb_moments": lb # dict keyed by power
    }
...
def dgt_lb_interface(
    ...
    for m in (1, 2, 3, 4):
        ...
        _, row_lb = acf_lb_interface(series_m, k=K, alpha=alpha)
        rows.append({
            "moment": m,
            "T": int(row_lb["T"]),
            "k": int(row_lb["k"]),
