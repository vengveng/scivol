...
# Native Python (default) implementation
...
print(result)
print(f'{result.x[0]:.4f} {result.x[1]:.4f} {result.x[2]:.4f}')

# Native Python (optimized) implementation
...
def log_likelihood() -> float:
        log_likelihood = constant_ll - 0.5 * (np.sum(np.log(sigma2) + residuals2 / sigma2))
        return -log_likelihood
