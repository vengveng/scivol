 ...
 *
 * Joint ARMA(p,q)-GJR-GARCH(P,Q) likelihoods with analytical derivatives.
 */
...
static inline int ged_precompute_full(double nu, ged_cache_t *cache)
{
    ...
    const double dlog_scale = 0.5 * (-digamma_inv + 3.0 * digamma_3inv) * inv_nu2;
    const double d2log_scale = 0.5 * (
        (trigamma_inv - 9.0 * trigamma_3inv) * inv_nu4
        + 2.0 * (digamma_inv - 3.0 * digamma_3inv) * inv_nu3
    );

    const double log_const = log(nu) - log(2.0) - log_scale - lgamma_approx(inv_nu);
    const double dlog_const = inv_nu - dlog_scale + digamma_inv * inv_nu2;
    const double d2log_const = -inv_nu2 - d2log_scale - trigamma_inv * inv_nu4 - 2.0 * digamma_inv * inv_nu3;
...
)
{
    ...
    const double A = exp(cache->nu * log_ratio);
    const double S = log_ratio - cache->nu * cache->dlog_scale;
    const double S_nu = -2.0 * cache->dlog_scale - cache->nu * cache->d2log_scale;

    if (!isfinite(A) || !isfinite(S) || !isfinite(S_nu)) {
        return 0;
    }

    *value = -cache->log_const + 0.5 * log(h_safe) + A;
    *ell_e = cache->nu * A * sign_e * inv_abs_e;
    *ell_h = 0.5 * (1.0 - cache->nu * A) * inv_h;
    *ell_nu = -cache->dlog_const + A * S;
    *ell_ee = cache->nu * (cache->nu - 1.0) * A * inv_abs_e2;
    *ell_eh = -0.5 * cache->nu * cache->nu * A * sign_e * inv_abs_e * inv_h;
    *ell_hh = (-0.5 + 0.25 * cache->nu * (cache->nu + 2.0) * A) * inv_h2;
    *ell_e_nu = A * sign_e * inv_abs_e * (1.0 + cache->nu * S);
    *ell_h_nu = -0.5 * A * (1.0 + cache->nu * S) * inv_h;
    *ell_nu_nu = -cache->d2log_const + A * (S * S + S_nu);
    return 1;
}

static inline size_t arma_gjr_garch_max_lag(
    size_t p_ar,
    size_t q_ma,
    size_t P_arch,
    size_t Q_garch
...
/* ========================================================================== */
/* Generic ARMA(p,q)-GJR-GARCH(P,Q) + Normal                                  */
/* ========================================================================== */
...
double arma_gjr_garch_nll_pq_normal(
    ...
    size_t max_lag = arma_gjr_garch_max_lag(p_ar, q_ma, P_arch, Q_garch);
    if (n <= max_lag) return 1e10;
    size_t n_eff = n - max_lag;

    size_t idx = 0;
    double c = params[idx++];
    const double *phi = params + idx; idx += p_ar;
    const double *theta = params + idx; idx += q_ma;
    double omega = params[idx++];
    const double *alpha = params + idx; idx += P_arch;
    const double *gamma = params + idx; idx += P_arch;
    const double *beta = params + idx;

    if (omega <= 0.0) return 1e10;

    double alpha_sum = 0.0, gamma_sum = 0.0, beta_sum = 0.0;
    for (size_t i = 0; i < P_arch; ++i) {
        if (alpha[i] < 0.0 || gamma[i] < 0.0) return 1e10;
        alpha_sum += alpha[i];
        gamma_sum += gamma[i];
    }
    ...
    if (alpha_sum + gamma_sum + beta_sum >= 1.0) return 1e10;
    ...
    for (size_t t = max_lag; t < n; ++t) {
        double mu_t = c;
        for (size_t i = 0; i < p_ar; ++i) mu_t += phi[i] * y[t - 1 - i];
        for (size_t j = 0; j < q_ma; ++j) mu_t += theta[j] * resid[t - 1 - j];
        resid[t] = y[t] - mu_t;
        ...
        for (size_t i = 0; i < P_arch; ++i) {
            ...
            h_t += alpha[i] * e2_lag + gamma[i] * ind * e2_lag;
        }
        for (size_t j = 0; j < Q_garch; ++j) h_t += beta[j] * sigma2[t - 1 - j];
        sigma2[t] = h_t;

        if (h_t < H_FLOOR || !isfinite(h_t)) return 1e10;
...
double arma_gjr_garch_nll_grad_pq_normal(
    ...
    const size_t max_lag = arma_gjr_garch_max_lag(p_ar, q_ma, P_arch, Q_garch);
    ...
    const size_t gamma_idx = alpha_idx + P_arch;
    const size_t beta_idx = gamma_idx + P_arch;
    ...
    if (de_hist == NULL || dh_hist == NULL || de_curr == NULL || dh_curr == NULL) {
        free(de_hist); free(dh_hist); free(de_curr); free(dh_curr);
        return 1e10;
    }
    ...
    for (size_t t = max_lag; t < n; ++t) {
        ...
        dzeros(de_curr, K);
        de_curr[0] = -1.0;
        for (size_t j = 0; j < q_ma; ++j) de_curr[0] -= theta[j] * de_hist[j * K];
        ...
        for (size_t k = 0; k < K; ++k) {
            ...
            for (size_t i = 0; i < P_arch; ++i) {
                ...
                const double e2_lag = e_lag * e_lag;
                const double ind = (e_lag < 0.0) ? 1.0 : 0.0;
                const double coeff = alpha[i] + gamma[i] * ind;
                val += coeff * (2.0 * e_lag * de_hist[i * K + k]);
                if (k == alpha_idx + i) val += e2_lag;
                if (k == gamma_idx + i) val += ind * e2_lag;
            }
            for (size_t j = 0; j < Q_garch; ++j) {
                val += beta[j] * dh_hist[j * K + k];
                if (k == beta_idx + j) val += sigma2[t - 1 - j];
            }
            dh_curr[k] = val;
        }

        {
            ...
            for (size_t k = 0; k < K; ++k) grad[k] += ell_e * de_curr[k] + ell_h * dh_curr[k];
        }
        sum_nll += normal_nll_var(resid[t], h_t);

        if (de_lags > 1) memmove(de_hist + K, de_hist, (de_lags - 1) * K * sizeof(double));
        memcpy(de_hist, de_curr, K * sizeof(double));
        if (dh_lags > 1) memmove(dh_hist + K, dh_hist, (dh_lags - 1) * K * sizeof(double));
...
void arma_gjr_garch_hess_pq_normal(
    ...
    double alpha_sum = 0.0, gamma_sum = 0.0, beta_sum = 0.0;
    for (size_t i = 0; i < P_arch; ++i) {
        if (alpha[i] < 0.0 || gamma[i] < 0.0) return;
        alpha_sum += alpha[i];
        gamma_sum += gamma[i];
    }
    for (size_t j = 0; j < Q_garch; ++j) {
    ...
    if (alpha_sum + gamma_sum + beta_sum >= 1.0) return;
    ...
    if (de_hist == NULL || dh_hist == NULL || d2e_hist == NULL || d2h_hist == NULL ||
        ...
        free(de_hist); free(dh_hist); free(d2e_hist); free(d2h_hist);
        free(de_curr); free(dh_curr); free(d2e_curr); free(d2h_curr);
        return;
    }
    ...
    for (size_t t = max_lag; t < n; ++t) {
        ...
        for (size_t i = 0; i < p_ar; ++i) {
            const size_t param_idx = 1 + i;
            de_curr[param_idx] = -y[t - 1 - i];
            for (size_t j = 0; j < q_ma; ++j) de_curr[param_idx] -= theta[j] * de_hist[j * K + param_idx];
        }

        for (size_t j = 0; j < q_ma; ++j) {
            const size_t param_idx = 1 + p_ar + j;
            de_curr[param_idx] = -resid[t - 1 - j];
            for (size_t l = 0; l < q_ma; ++l) de_curr[param_idx] -= theta[l] * de_hist[l * K + param_idx];
        }

        for (size_t k = omega_idx; k < K; ++k) {
            for (size_t j = 0; j < q_ma; ++j) de_curr[k] -= theta[j] * de_hist[j * K + k];
        }
        ...
        for (size_t l = 0; l < q_ma; ++l) {
            ...
            for (size_t ij = 0; ij < K * K; ++ij) d2e_curr[ij] -= theta_l * d2e_lag[ij];
        ...
        for (size_t i = 0; i < P_arch; ++i) {
            ...
            for (size_t b = 0; b < K; ++b) {
                ...
                d2h_curr[gamma_param_idx * K + b] += ind * de2_lag_b;
                d2h_curr[b * K + gamma_param_idx] += ind * de2_lag_b;
            }
        }
        ...
        for (size_t j = 0; j < Q_garch; ++j) {
            ...
            for (size_t idx_ab = 0; idx_ab < K * K; ++idx_ab) d2h_curr[idx_ab] += beta[j] * d2h_lag[idx_ab];
            for (size_t b = 0; b < K; ++b) {
    ...
    const double scale = 1.0 / (double)n_eff;
    for (size_t idx_h = 0; idx_h < K * K; ++idx_h) hess[idx_h] *= scale;
    free(de_hist); free(dh_hist); free(d2e_hist); free(d2h_hist);
    free(de_curr); free(dh_curr); free(d2e_curr); free(d2h_curr);
}

/* ========================================================================== */
/* Generic ARMA(p,q)-GJR-GARCH(P,Q) + Student-t                               */
/* ========================================================================== */
...
double arma_gjr_garch_nll_pq_studentt(
    ...
    if (n <= max_lag) return 1e10;
    const size_t n_eff = n - max_lag;
    const size_t K_base = 1 + p_ar + q_ma + 1 + 2 * P_arch + Q_garch;
    ...
    const double cnst = lgamma_approx(0.5 * (nu + 1.0)) - lgamma_approx(0.5 * nu) - 0.5 * log(nu * M_PI);
    double sum_nll = 0.0;
    for (size_t t = max_lag; t < n; ++t) {
        double mu_t = c;
        for (size_t i = 0; i < p_ar; ++i) mu_t += phi[i] * y[t - 1 - i];
...
double arma_gjr_garch_nll_grad_pq_studentt(
    ...
    if (omega <= 0.0 || nu <= NU_MIN) return 1e10;
    double alpha_sum = 0.0, gamma_sum = 0.0, beta_sum = 0.0;
    for (size_t i = 0; i < P_arch; ++i) {
        if (alpha[i] < 0.0 || gamma[i] < 0.0) return 1e10;
        alpha_sum += alpha[i];
    ...
    const double inv_nu = 1.0 / nu;
    const double psi_half_nu_plus_1 = digamma_approx(0.5 * (nu + 1.0));
    ...
    for (size_t t = max_lag; t < n; ++t) {
        ...
        de_curr[0] = -1.0;
        for (size_t j = 0; j < q_ma; ++j) de_curr[0] -= theta[j] * de_hist[j * K_base];
        for (size_t i = 0; i < p_ar; ++i) {
            const size_t param_idx = 1 + i;
            de_curr[param_idx] = -y[t - 1 - i];
            for (size_t j = 0; j < q_ma; ++j) de_curr[param_idx] -= theta[j] * de_hist[j * K_base + param_idx];
        }
        for (size_t j = 0; j < q_ma; ++j) {
            const size_t param_idx = 1 + p_ar + j;
            de_curr[param_idx] = -resid[t - 1 - j];
            for (size_t l = 0; l < q_ma; ++l) de_curr[param_idx] -= theta[l] * de_hist[l * K_base + param_idx];
        }
        for (size_t k = omega_idx; k < K_base; ++k) {
            for (size_t j = 0; j < q_ma; ++j) de_curr[k] -= theta[j] * de_hist[j * K_base + k];
        }
        ...
        for (size_t k = 0; k < K_base; ++k) {
            ...
            for (size_t i = 0; i < P_arch; ++i) {
                ...
                const double ind = (e_lag < 0.0) ? 1.0 : 0.0;
                const double coeff = alpha[i] + gamma[i] * ind;
                val += coeff * (2.0 * e_lag * de_hist[i * K_base + k]);
                if (k == alpha_idx + i) val += e2_lag;
                if (k == gamma_idx + i) val += ind * e2_lag;
            }
            for (size_t j = 0; j < Q_garch; ++j) {
        ...
        const double e_t = resid[t];
        const double z2 = e_t * e_t / h_t;
        const double one_plus_z2_over_nu = 1.0 + z2 * inv_nu;
        const double ell_e = (nu + 1.0) * e_t / (h_t * (nu + z2));
        const double ell_h = 0.5 / h_t - 0.5 * (nu + 1.0) * z2 / (h_t * (nu + z2));
        const double grad_nu = -0.5 * psi_half_nu_plus_1 + 0.5 * psi_half_nu
                             + 0.5 * inv_nu
                             + 0.5 * log(one_plus_z2_over_nu)
                             - 0.5 * (nu + 1.0) * z2 * inv_nu / (nu + z2);

        for (size_t k = 0; k < K_base; ++k) grad[k] += ell_e * de_curr[k] + ell_h * dh_curr[k];
        grad[nu_idx] += grad_nu;
        sum_nll += studentt_nll_var(e_t, h_t, nu);

        if (de_lags > 1) memmove(de_hist + K_base, de_hist, (de_lags - 1) * K_base * sizeof(double));
        memcpy(de_hist, de_curr, K_base * sizeof(double));
        if (dh_lags > 1) memmove(dh_hist + K_base, dh_hist, (dh_lags - 1) * K_base * sizeof(double));
...
void arma_gjr_garch_hess_pq_studentt(
    ...
    for (size_t t = max_lag; t < n; ++t) {
        ...
        for (size_t l = 0; l < q_ma; ++l) {
            ...
            for (size_t ij = 0; ij < K_base * K_base; ++ij) d2e_curr[ij] -= theta_l * d2e_lag[ij];
        ...
        for (size_t i = 0; i < P_arch; ++i) {
            ...
            for (size_t a = 0; a < K_base; ++a) {
                for (size_t b = 0; b < K_base; ++b) {
                    ...
                    d2h_curr[ij] += coeff * 2.0 * (de_lag[a] * de_lag[b] + e_lag * d2e_lag[ij]);
                }
            }
            for (size_t j = 0; j < K_base; ++j) {
                ...
                d2h_curr[gamma_param_idx * K_base + j] += ind * cross;
                d2h_curr[j * K_base + gamma_param_idx] += ind * cross;
            }
        }
        ...
        for (size_t j = 0; j < Q_garch; ++j) {
            ...
            const size_t beta_param_idx = beta_idx + j;
            for (size_t ij = 0; ij < K_base * K_base; ++ij) d2h_curr[ij] += beta[j] * d2h_lag[ij];
            for (size_t k = 0; k < K_base; ++k) {
    ...
    const double scale = 1.0 / (double)n_eff;
    for (size_t ij = 0; ij < K * K; ++ij) hess[ij] *= scale;
    free(de_hist); free(dh_hist); free(d2e_hist); free(d2h_hist);
    free(de_curr); free(dh_curr); free(d2e_curr); free(d2h_curr);
}

/* ========================================================================== */
/* Generic ARMA(p,q)-GJR-GARCH(P,Q) + GED                                     */
/* ========================================================================== */
...
double arma_gjr_garch_nll_pq_ged(
    ...
    ged_cache_t cache;
    if (omega <= 0.0 || !ged_precompute_full(nu, &cache)) return 1e10;
    double alpha_sum = 0.0, gamma_sum = 0.0, beta_sum = 0.0;
    for (size_t i = 0; i < P_arch; ++i) {
        if (alpha[i] < 0.0 || gamma[i] < 0.0) return 1e10;
        alpha_sum += alpha[i];
    ...
    for (size_t t = max_lag; t < n; ++t) {
        ...
        double value, ell_e, ell_h, ell_nu, ell_ee, ell_eh, ell_hh, ell_e_nu, ell_h_nu, ell_nu_nu;
        if (!ged_obs_derivs(resid[t], h_t, &cache, &value, &ell_e, &ell_h, &ell_nu, &ell_ee, &ell_eh, &ell_hh, &ell_e_nu, &ell_h_nu, &ell_nu_nu)) {
            return 1e10;
        }
        sum_nll += value;
    }
...
void arma_gjr_garch_hess_pq_ged(
    ...
    ged_cache_t cache;
    if (omega <= 0.0 || !ged_precompute_full(nu, &cache)) return;
    double alpha_sum = 0.0, gamma_sum = 0.0, beta_sum = 0.0;
    for (size_t i = 0; i < P_arch; ++i) {
        if (alpha[i] < 0.0 || gamma[i] < 0.0) return;
        alpha_sum += alpha[i];
    ...
    for (size_t t = max_lag; t < n; ++t) {
        ...
        for (size_t i = 0; i < K_base; ++i) {
            ...
            const double cross = ell_e_nu * de_curr[i] + ell_h_nu * dh_curr[i];
            hess[i * K + nu_idx] += cross;
            hess[nu_idx * K + i] += cross;
        }
        hess[nu_idx * K + nu_idx] += ell_nu_nu;
...
/* ========================================================================== */
/* Generic ARMA(p,q)-GJR-GARCH(P,Q) + Skew-t                                  */
/* ========================================================================== */
...
double arma_gjr_garch_nll_pq_skewt(
    ...
    const double lgamma_c = lgamma_approx(0.5 * (nu + 1.0)) - lgamma_approx(0.5 * nu);
    const double base_cnst = log(b) + lgamma_c - 0.5 * log((nu - 2.0) * M_PI);
    double sum_nll = 0.0;
...
double arma_gjr_garch_nll_grad_pq_skewt(
    ...
    if (omega <= 0.0 || nu <= NU_MIN || fabs(lam) >= LAM_MAX) return 1e10;
    double alpha_sum = 0.0, gamma_sum = 0.0, beta_sum = 0.0;
    for (size_t i = 0; i < P_arch; ++i) {
        if (alpha[i] < 0.0 || gamma[i] < 0.0) return 1e10;
        alpha_sum += alpha[i];
    ...
    skewt_cache_t cache;
    if (!skewt_precompute_full(nu, lam, &cache)) return 1e10;
    ...
    for (size_t t = max_lag; t < n; ++t) {
        ...
        for (size_t k = 0; k < K_base; ++k) grad[k] += obs.ell_e * de_curr[k] + obs.ell_h * dh_curr[k];
...
void arma_gjr_garch_hess_pq_skewt(
    ...
    if (omega <= 0.0 || nu <= NU_MIN || fabs(lam) >= LAM_MAX) return;
    double alpha_sum = 0.0, gamma_sum = 0.0, beta_sum = 0.0;
    for (size_t i = 0; i < P_arch; ++i) {
        if (alpha[i] < 0.0 || gamma[i] < 0.0) return;
        alpha_sum += alpha[i];
    ...
    skewt_cache_t cache;
    if (!skewt_precompute_full(nu, lam, &cache)) return;
    ...
    for (size_t t = max_lag; t < n; ++t) {
        ...
        for (size_t i = 0; i < K_base; ++i) {
            for (size_t j = 0; j < K_base; ++j) {
                const size_t idx_h = i * K + j;
                const size_t idx_base = i * K_base + j;
                hess[idx_h] += obs.ell_ee * de_curr[i] * de_curr[j]
...
/* ========================================================================== */
/* ARMA(1,1)-GJR-GARCH(1,1) wrappers                                          */
/* ========================================================================== */
...
double arma_gjr_garch_nll_11_normal(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    return arma_gjr_garch_nll_pq_normal(params, y, resid, sigma2, e0_arr, h0_arr, n, 1, 1, 1, 1);
}
...
double arma_gjr_garch_nll_grad_11_normal(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    return arma_gjr_garch_nll_grad_pq_normal(params, y, resid, sigma2, e0_arr, h0_arr, grad, n, 1, 1, 1, 1);
}
...
void arma_gjr_garch_hess_11_normal(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    arma_gjr_garch_hess_pq_normal(params, y, resid, sigma2, e0_arr, h0_arr, hess, n, 1, 1, 1, 1);
}
...
double arma_gjr_garch_nll_11_studentt(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    return arma_gjr_garch_nll_pq_studentt(params, y, resid, sigma2, e0_arr, h0_arr, n, 1, 1, 1, 1);
}
...
double arma_gjr_garch_nll_grad_11_studentt(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    return arma_gjr_garch_nll_grad_pq_studentt(params, y, resid, sigma2, e0_arr, h0_arr, grad, n, 1, 1, 1, 1);
}
...
void arma_gjr_garch_hess_11_studentt(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    arma_gjr_garch_hess_pq_studentt(params, y, resid, sigma2, e0_arr, h0_arr, hess, n, 1, 1, 1, 1);
}
...
double arma_gjr_garch_nll_11_ged(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    return arma_gjr_garch_nll_pq_ged(params, y, resid, sigma2, e0_arr, h0_arr, n, 1, 1, 1, 1);
}
...
double arma_gjr_garch_nll_grad_11_ged(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    return arma_gjr_garch_nll_grad_pq_ged(params, y, resid, sigma2, e0_arr, h0_arr, grad, n, 1, 1, 1, 1);
}
...
void arma_gjr_garch_hess_11_ged(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    arma_gjr_garch_hess_pq_ged(params, y, resid, sigma2, e0_arr, h0_arr, hess, n, 1, 1, 1, 1);
}
...
double arma_gjr_garch_nll_11_skewt(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    return arma_gjr_garch_nll_pq_skewt(params, y, resid, sigma2, e0_arr, h0_arr, n, 1, 1, 1, 1);
}
...
double arma_gjr_garch_nll_grad_11_skewt(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    return arma_gjr_garch_nll_grad_pq_skewt(params, y, resid, sigma2, e0_arr, h0_arr, grad, n, 1, 1, 1, 1);
}
...
void arma_gjr_garch_hess_11_skewt(
    ...
    double e0_arr[1] = {0.0};
    double h0_arr[1] = {h0};
    arma_gjr_garch_hess_pq_skewt(params, y, resid, sigma2, e0_arr, h0_arr, hess, n, 1, 1, 1, 1);
}
