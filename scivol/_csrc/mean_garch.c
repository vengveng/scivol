...
VLK_INLINE double lm_sigma2_pq(
    double omega,
...
VLK_INLINE void lm_hess_accumulate(double *restrict H, size_t i, size_t j, size_t K, double v)
{
    H[i * K + j] += v;
    if (j != i) {
        H[j * K + i] += v;
...
VLK_INLINE double lm_dot(const double *restrict a, const double *restrict b, size_t n)
{
    double out = 0.0;
...
VLK_INLINE int lm_validate_garch(double omega,
                                 const double *restrict alpha,
                                 const double *restrict beta,
                                 size_t p,
                                 size_t q)
{
    ...
    for (size_t i = 0; i < p; ++i) {
        if (alpha[i] < 0.0 || !isfinite(alpha[i])) {
            return 0;
        }
        alpha_sum += alpha[i];
    }
    ...
    return alpha_sum + beta_sum < 1.0;
}

VLK_INLINE void lm_update_dynamic_grad(
    double *restrict D_t,
...
)
{
    ...
    const size_t beta_base = alpha_base + p;
    ...
    for (size_t j = 1; j <= p && t >= j; ++j) {
        ...
        for (size_t i = 0; i < n_mean; ++i) {
            D_t[i] -= 2.0 * alpha[j - 1] * e_lag * f_lag[i];
        }
...
VLK_INLINE void lm_update_dynamic_hess(
    double *restrict D_t,
    double *restrict C_t,
    double *restrict D_buf,
...
)
{
    ...
    lm_update_dynamic_grad(D_t, D_buf, ring, features, resid, sigma2, alpha, beta, t, n_mean, p, q, ndyn);
    ...
    for (size_t j = 1; j <= p && t >= j; ++j) {
        ...
        for (size_t a = 0; a < n_mean; ++a) {
            ...
            for (size_t b = 0; b < n_mean; ++b) {
                C_t[a * ndyn + b] += 2.0 * alpha[j - 1] * ga * (-f_lag[b]);
            }
        }

        {
            ...
            for (size_t b = 0; b < n_mean; ++b) {
                const double cross = -2.0 * e_lag * f_lag[b];
                C_t[a_idx * ndyn + b] += cross;
                C_t[b * ndyn + a_idx] += cross;
            }
        }
    }
...
VLK_INLINE int lm_studentt_obs_derivs(
    double e,
    double h,
    double nu,
    lm_studentt_obs_derivs_t *out
)
{
...
VLK_INLINE int lm_ged_obs_derivs(
    double e,
    double h,
    double nu,
    const lm_ged_cache_t *cache,
    lm_ged_obs_derivs_t *out
)
{
...
VLK_INLINE int lm_skewt_obs_derivs(
    double e,
    double h,
    double nu,
    double lam,
    const lm_skewt_cache_t *cache,
    lm_skewt_obs_derivs_t *out
)
{
...
double linear_mean_garch_nll_pq_normal(
    const double *params,
    const double *y,
    ...
    const size_t beta_idx = alpha_idx + p;
    ...
    for (size_t t = 0; t < n; ++t) {
        ...
        resid[t] = y[t] - lm_dot(ft, mean_params, n_mean);
        if (p == 0 && q == 0) {
            sigma2[t] = omega;
        } else if (t > 0) {
            sigma2[t] = lm_sigma2_pq(omega, alpha, beta, p, q, t, resid, sigma2);
        }
...
double linear_mean_garch_nll_11_normal(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    return linear_mean_garch_nll_pq_normal(params, y, features, resid, sigma2, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
void linear_mean_garch_nll_grad_pq_normal(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
...
__attribute__((visibility("default"), hot, flatten))
void linear_mean_garch_nll_grad_11_normal(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    linear_mean_garch_nll_grad_pq_normal(params, y, features, resid, sigma2, grad, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
void linear_mean_garch_hess_pq_normal(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
        ...
        if (t > 0) {
            ...
            for (size_t j = 1; j <= p && t >= j; ++j) {
                ...
                for (size_t a = 0; a < n_mean; ++a) {
                    const double ga = -f_lag[a];
                    for (size_t b = 0; b < n_mean; ++b) {
                        C_t[a * K + b] += 2.0 * alpha[j - 1] * ga * (-f_lag[b]);
                    }
                }
...
__attribute__((visibility("default"), hot, flatten))
void linear_mean_garch_hess_11_normal(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    linear_mean_garch_hess_pq_normal(params, y, features, resid, sigma2, hess, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
double linear_mean_garch_nll_pq_studentt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    if (!lm_validate_garch(omega, alpha, beta, p, q) || !(nu > NU_MIN) || !isfinite(nu)) {
        return 1e10;
    }
    ...
    for (size_t t = 0; t < n; ++t) {
        ...
        if (!lm_studentt_obs_derivs(resid[t], sigma2[t], nu, &obs)) {
            return 1e10;
        }
        nll += obs.value;
    }
...
__attribute__((visibility("default"), hot, flatten))
double linear_mean_garch_nll_11_studentt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    return linear_mean_garch_nll_pq_studentt(params, y, features, resid, sigma2, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
void linear_mean_garch_nll_grad_pq_studentt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    const size_t ndyn = n_mean + 1 + p + q;
...
__attribute__((visibility("default"), hot, flatten))
void linear_mean_garch_nll_grad_11_studentt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    linear_mean_garch_nll_grad_pq_studentt(params, y, features, resid, sigma2, grad, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
void linear_mean_garch_hess_pq_studentt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    for (size_t t = 0; t < n; ++t) {
        ...
        lm_update_dynamic_hess(D_t, C_t, D_buf, C_buf, ring, features, resid, sigma2, alpha, beta, t, n_mean, p, q, ndyn);
        ...
        lm_hess_accumulate(hess, ndyn, ndyn, K, obs.ell_nu_nu);
    }
...
__attribute__((visibility("default"), hot, flatten))
void linear_mean_garch_hess_11_studentt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    linear_mean_garch_hess_pq_studentt(params, y, features, resid, sigma2, hess, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
double linear_mean_garch_nll_pq_ged(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    const double *beta = params + beta_idx;
    const double nu = params[nu_idx];
    lm_ged_cache_t cache;

    if (!lm_validate_garch(omega, alpha, beta, p, q) || !lm_ged_precompute(nu, &cache)) {
        return 1e10;
    }
    ...
    for (size_t t = 0; t < n; ++t) {
        ...
        if (!lm_ged_obs_derivs(resid[t], sigma2[t], nu, &cache, &obs)) {
            return 1e10;
        }
        nll += obs.value;
    }
...
__attribute__((visibility("default"), hot, flatten))
double linear_mean_garch_nll_11_ged(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    return linear_mean_garch_nll_pq_ged(params, y, features, resid, sigma2, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
void linear_mean_garch_nll_grad_pq_ged(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
...
__attribute__((visibility("default"), hot, flatten))
void linear_mean_garch_nll_grad_11_ged(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    linear_mean_garch_nll_grad_pq_ged(params, y, features, resid, sigma2, grad, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
void linear_mean_garch_hess_pq_ged(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
...
__attribute__((visibility("default"), hot, flatten))
void linear_mean_garch_hess_11_ged(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    linear_mean_garch_hess_pq_ged(params, y, features, resid, sigma2, hess, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
double linear_mean_garch_nll_pq_skewt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    lm_skewt_cache_t cache;

    if (!lm_validate_garch(omega, alpha, beta, p, q) || !lm_skewt_precompute_full(nu, lam, &cache)) {
        return 1e10;
    }
    ...
    for (size_t t = 0; t < n; ++t) {
        ...
        if (!lm_skewt_obs_derivs(resid[t], sigma2[t], nu, lam, &cache, &obs)) {
            return 1e10;
        }
        nll += obs.value;
    }
...
__attribute__((visibility("default"), hot, flatten))
double linear_mean_garch_nll_11_skewt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    return linear_mean_garch_nll_pq_skewt(params, y, features, resid, sigma2, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
void linear_mean_garch_nll_grad_pq_skewt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
...
__attribute__((visibility("default"), hot, flatten))
void linear_mean_garch_nll_grad_11_skewt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    linear_mean_garch_nll_grad_pq_skewt(params, y, features, resid, sigma2, grad, n, n_mean, 1, 1);
}

__attribute__((visibility("default"), hot))
void linear_mean_garch_hess_pq_skewt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    for (size_t t = 0; t < n; ++t) {
        ...
        lm_hess_accumulate(hess, nu_idx, nu_idx, K, obs.ell_nu_nu);
        lm_hess_accumulate(hess, nu_idx, lam_idx, K, obs.ell_nu_lam);
        lm_hess_accumulate(hess, lam_idx, lam_idx, K, obs.ell_lam_lam);
    }
...
__attribute__((visibility("default"), hot, flatten))
void linear_mean_garch_hess_11_skewt(
    const double *params,
    const double *y,
    const double *features,
    double       *resid,
    ...
    linear_mean_garch_hess_pq_skewt(params, y, features, resid, sigma2, hess, n, n_mean, 1, 1);
}
