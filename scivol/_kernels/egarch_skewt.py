...
_UID_RE = re.compile(r"^EGARCH\((\d+),(\d+)\)\+SkewT$")
...
def _build(p: int, q: int) -> Routine:
    uid = f"EGARCH({p},{q})+SkewT"
    vol = EGARCH(p, q)
    dens = SkewT()
    spec = CompositeSpec(vol, dens)

    special = p == 1 and q == 1
    c_obj = _core._egarch_ll_11_skewt if special else _core._egarch_ll_pq_skewt
    c_jac = _core._egarch_ll_grad_11_skewt if special else _core._egarch_ll_grad_pq_skewt
    c_hess = _core._egarch_ll_hess_11_skewt if special else _core._egarch_ll_hess_pq_skewt
    c_log_obj = _core._log_egarch_ll_11_skewt if special else _core._log_egarch_ll_pq_skewt
    c_log_jac = _core._log_egarch_ll_grad_11_skewt if special else _core._log_egarch_ll_grad_pq_skewt
        ...
        **_,
        ...
        if not log_mode:
            ...
            def pack_theta(z: NDArray[np.float64]) -> NDArray[np.float64]:
                pack_egarch_skewt_c(z, theta_buf, p, q)
            ...
            def hessian_log(z: NDArray[np.float64]) -> NDArray[np.float64]:
                ...
                return log_hessian_egarch(theta, grad_theta, hess_theta, p, q, dist="skewt") / n

            z0 = unpack_egarch_skewt(np.concatenate((vol.default_start(resid), dens.default_start(resid))), p, q)
        ...
        def hess_z(z: NDArray[np.float64]) -> NDArray[np.float64]:
            theta = pack_egarch_skewt(z, p, q)
            grad_theta = gradient(theta) * n
            hess_theta = hessian(theta) * n
            return log_hessian_egarch(theta, grad_theta, hess_theta, p, q, dist="skewt")

        H_theta, cov_matrix = compute_se_via_logspace(
            theta_hat=theta_hat,
            nll_theta=nll_theta,
            unpack_fn=lambda theta: unpack_egarch_skewt(theta, p, q),
            jacobian_fn=lambda theta: jacobian_egarch_skewt(theta, p, q),
            pack_fn=lambda z: pack_egarch_skewt(z, p, q),
            hess_z_fn=hess_z,
        )
    ...
    return Routine(uid=uid, fit=fit, n_params=3 + 2 * p + q)


def get_routine(uid: str) -> Routine:
    match = _UID_RE.fullmatch(uid)
    if not match:
        raise RuntimeError(f"Unsupported UID '{uid}' for EGARCH + SkewT.")
