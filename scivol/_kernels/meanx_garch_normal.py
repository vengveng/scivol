...
_RE_ARX = re.compile(r"ARX\((\d+)\)\+GARCH\((\d+),(\d+)\)\+(Normal|StudentT|SkewT|GED)$")
_RE_HARX = re.compile(r"HARX\(([0-9,]+)\)\+GARCH\((\d+),(\d+)\)\+(Normal|StudentT|SkewT|GED)$")
...
def _parse_uid(uid: str) -> tuple[ARX | HARX, int, int, str]:
    ...
    raise ValueError(f"Unsupported uid for meanx_garch_normal: {uid}")


def _build(mean: ARX | HARX, p: int, q: int, density_name: str) -> Routine:
    ops = _DENSITY_OPS[density_name]
    uid = f"{mean.signature}+GARCH({p},{q})+{density_name}"
    vol = GARCH(p, q)
    dens = ops.factory()
    ...
    def fit(
        ...
        **_,
        ...
        def call_nll(theta: NDArray[np.float64]) -> float:
            sigma2[0] = np.mean(y_arr * y_arr)
            if use_specialized:
                return getattr(_core, f"_linear_mean_garch_nll_11_{ops.suffix}")(
        ...
        def call_grad(theta: NDArray[np.float64]) -> NDArray[np.float64]:
            sigma2[0] = np.mean(y_arr * y_arr)
            if use_specialized:
                getattr(_core, f"_linear_mean_garch_nll_grad_11_{ops.suffix}")(
        ...
        def call_hess(theta: NDArray[np.float64]) -> NDArray[np.float64]:
            sigma2[0] = np.mean(y_arr * y_arr)
            if use_specialized:
                getattr(_core, f"_linear_mean_garch_hess_11_{ops.suffix}")(
        ...
        vol_start = np.concatenate([[np.mean(y_arr * y_arr) * 0.1], [0.05 / p] * p, [0.90 / q] * q]).astype(np.float64)
        dens_start = np.asarray(dens.default_start(y_arr), dtype=np.float64)
        start = np.concatenate([mean_start, vol_start, dens_start])

        mean_bounds = list(mean.bounds())
        bounds = mean_bounds + [(1e-10, None)] + [(1e-10, 0.999)] * p + [(1e-10, 0.999)] * q + list(dens.bounds())

        A = np.zeros((1, n_params), dtype=np.float64)
        A[0, n_mean + 1:n_mean + 1 + p] = 1.0
        A[0, n_mean + 1 + p:n_mean + 1 + p + q] = 1.0
