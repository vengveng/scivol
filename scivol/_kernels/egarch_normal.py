...
_UID_RE = re.compile(r"^EGARCH\((\d+),(\d+)\)\+Normal$")
...
def _build(p: int, q: int) -> Routine:
    uid = f"EGARCH({p},{q})+Normal"
    vol = EGARCH(p, q)
    dens = Normal()
    spec = CompositeSpec(vol, dens)

    special = p == 1 and q == 1
    c_obj = _core._egarch_ll_11_normal if special else _core._egarch_ll_pq_normal
    c_jac = _core._egarch_ll_grad_11_normal if special else _core._egarch_ll_grad_pq_normal
    c_hess = _core._egarch_ll_hess_11_normal if special else _core._egarch_ll_hess_pq_normal
    c_log_obj = _core._log_egarch_ll_11_normal if special else _core._log_egarch_ll_pq_normal
    c_log_jac = _core._log_egarch_ll_grad_11_normal if special else _core._log_egarch_ll_grad_pq_normal
    ...
    def fit(
        ...
        **_,
        ...
        constant_nll = 0.5 * n * np.log(2.0 * np.pi)
        ...
        if not log_mode:
            theta0 = vol.default_start(resid)
            bounds = vol.bounds()
            ...
            res.fun = float(res.fun) * n + constant_nll
        else:
            ...
            def hessian_log(z: NDArray[np.float64]) -> NDArray[np.float64]:
                ...
                return log_hessian_egarch(theta, grad_theta, hess_theta, p, q, dist="normal") / n

            z0 = unpack_egarch(vol.default_start(resid), p, q)
            ...
            res.fun = objective_log(z_hat) * n + constant_nll

        vol.unpack(theta_hat[:K])
        call_c_obj(theta_hat)

        def nll_theta(theta: NDArray[np.float64]) -> float:
            return call_c_obj(theta) + constant_nll

        def hess_z(z: NDArray[np.float64]) -> NDArray[np.float64]:
            theta = pack_egarch(z, p, q)
            grad_theta = gradient(theta) * n
            hess_theta = hessian(theta) * n
            return log_hessian_egarch(theta, grad_theta, hess_theta, p, q, dist="normal")

        H_theta, cov_matrix = compute_se_via_logspace(
            theta_hat=theta_hat,
            nll_theta=nll_theta,
            unpack_fn=lambda theta: unpack_egarch(theta, p, q),
            jacobian_fn=lambda theta: jacobian_egarch(theta, p, q),
            pack_fn=lambda z: pack_egarch(z, p, q),
            hess_z_fn=hess_z,
        )
    ...
    return Routine(uid=uid, fit=fit, n_params=1 + 2 * p + q)


def get_routine(uid: str) -> Routine:
    match = _UID_RE.fullmatch(uid)
    if not match:
        raise RuntimeError(f"Unsupported UID '{uid}' for EGARCH + Normal.")
