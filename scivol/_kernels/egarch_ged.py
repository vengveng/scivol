...
_UID_RE = re.compile(r"^EGARCH\((\d+),(\d+)\)\+GED$")
...
def _build(p: int, q: int) -> Routine:
    ...
    special = p == 1 and q == 1
    c_obj = _core._egarch_ll_11_ged if special else _core._egarch_ll_pq_ged
    c_jac = _core._egarch_ll_grad_11_ged if special else _core._egarch_ll_grad_pq_ged
    c_hess = _core._egarch_ll_hess_11_ged if special else _core._egarch_ll_hess_pq_ged
    c_log_obj = _core._log_egarch_ll_11_ged if special else _core._log_egarch_ll_pq_ged
    c_log_jac = _core._log_egarch_ll_grad_11_ged if special else _core._log_egarch_ll_grad_pq_ged
        ...
        **_,
        ...
        if not log_mode:
            ...
            def hessian_log(z: NDArray[np.float64]) -> NDArray[np.float64]:
                theta = pack_egarch_ged(z, p, q)
                call_c_jac(theta)
                grad_theta = grad_vec.copy()
                call_c_hess(theta)
                hess_theta = hess_mat.copy()
                return log_hessian_egarch(theta, grad_theta, hess_theta, p, q, dist="ged") / n

            z0 = unpack_egarch_ged(theta0, p, q)
            ...
            theta_hat = pack_egarch_ged(z_hat, p, q)
        ...
        H_theta, cov_matrix = compute_se_via_logspace(
            theta_hat=theta_hat,
            nll_theta=lambda theta: call_c_obj(theta),
            unpack_fn=lambda theta: unpack_egarch_ged(theta, p, q),
            jacobian_fn=lambda theta: jacobian_egarch_ged(theta, p, q),
            pack_fn=lambda z: pack_egarch_ged(z, p, q),
            hess_z_fn=lambda z: log_hessian_egarch(
                pack_egarch_ged(z, p, q),
                gradient(pack_egarch_ged(z, p, q)) * n,
                hessian(pack_egarch_ged(z, p, q)) * n,
                p,
                q,
                dist="ged",
            ),
...
def get_routine(uid: str) -> Routine:
    ...
    if not match:
        raise RuntimeError(f"Unsupported UID '{uid}' for EGARCH + GED.")
