...
from .transforms import (
    ...
    pack_arma_egarch_normal,
    unpack_arma_egarch_normal,
)

_CACHE: Dict[Tuple[int, int, int, int], Routine] = {}
_UID_RE = re.compile(r"^ARMA\((\d+),(\d+)\)\+EGARCH\((\d+),(\d+)\)\+Normal$")
...
def _build(p_ar: int, q_ma: int, p_arch: int, q_egarch: int) -> Routine:
    uid = f"ARMA({p_ar},{q_ma})+EGARCH({p_arch},{q_egarch})+Normal"
    mean = ARMA(p_ar, q_ma)
    ...
    def fit(
        ...
        **_,
        ...
        if special:
            grad_vec = np.zeros(7, dtype=np.float64)
            hess_mat = np.zeros((7, 7), dtype=np.float64)

            def call_nll(params: NDArray[np.float64]) -> float:
                return float(_core._arma_egarch_nll_11_normal(_as_cptr(params), y_c, resid_c, sigma2_c, h0, n))

            def call_grad(params: NDArray[np.float64]) -> NDArray[np.float64]:
                _core._arma_egarch_nll_grad_11_normal(
            ...
            def call_nll(params: NDArray[np.float64]) -> float:
                return float(
                    _core._arma_egarch_nll_pq_normal(
        ...
        bounds = [(-1.0, 1.0)] + [(-0.99, 0.99)] * p_ar + [(-0.99, 0.99)] * q_ma + vol.bounds()
        ...
        if not log_mode:
            ...
            def obj_log(z: NDArray[np.float64]) -> float:
                return float(
                    _core._log_arma_egarch_nll_pq_normal(
            ...
            def hess_log(z: NDArray[np.float64]) -> NDArray[np.float64]:
                theta_local = pack_arma_egarch_normal(z, p_ar, q_ma, p_arch, q_egarch)
                grad_theta = call_grad(theta_local) * p_scaler
                hess_theta = call_hess(theta_local) * p_scaler
                return log_hessian_arma_egarch_normal(
                    theta_local, grad_theta, hess_theta, p_ar, q_ma, p_arch, q_egarch
                )

            z0 = unpack_arma_egarch_normal(theta0, p_ar, q_ma, p_arch, q_egarch)
            if solver.lower() == "nelder-mead":
            ...
            theta_hat = pack_arma_egarch_normal(np.asarray(res.x, dtype=np.float64), p_ar, q_ma, p_arch, q_egarch)
        ...
        if log_mode:
            hessian_theta, cov_matrix = compute_se_via_logspace(
                ...
                unpack_fn=lambda theta: unpack_arma_egarch_normal(theta, p_ar, q_ma, p_arch, q_egarch),
                jacobian_fn=lambda theta: jacobian_arma_egarch_normal(theta, p_ar, q_ma, p_arch, q_egarch),
                pack_fn=lambda z: pack_arma_egarch_normal(z, p_ar, q_ma, p_arch, q_egarch),
                hess_z_fn=lambda z: log_hessian_arma_egarch_normal(
                    pack_arma_egarch_normal(z, p_ar, q_ma, p_arch, q_egarch),
                    call_grad(pack_arma_egarch_normal(z, p_ar, q_ma, p_arch, q_egarch)),
                    call_hess(pack_arma_egarch_normal(z, p_ar, q_ma, p_arch, q_egarch)),
                    p_ar,
                    q_ma,
                    p_arch,
                    q_egarch,
...
def get_routine(uid: str) -> Routine:
    ...
    if not match:
        raise RuntimeError(f"Unsupported UID '{uid}' for ARMA + EGARCH + Normal.")
