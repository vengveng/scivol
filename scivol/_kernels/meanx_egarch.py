...
from .linear_mean_egarch_common import (
    _as_cptr,
    build_linear_mean_features,
    jacobian_linear_mean_egarch_ged,
    jacobian_linear_mean_egarch_normal,
    jacobian_linear_mean_egarch_skewt,
    jacobian_linear_mean_egarch_studentt,
    log_hessian_linear_mean_egarch_ged,
    log_hessian_linear_mean_egarch_normal,
    log_hessian_linear_mean_egarch_skewt,
    log_hessian_linear_mean_egarch_studentt,
    pack_linear_mean_egarch_ged,
    pack_linear_mean_egarch_normal,
    pack_linear_mean_egarch_skewt,
    pack_linear_mean_egarch_studentt,
    unpack_linear_mean_egarch_ged,
    unpack_linear_mean_egarch_normal,
    unpack_linear_mean_egarch_skewt,
    unpack_linear_mean_egarch_studentt,
)
from .routine import Routine
from .transforms import compute_se_via_logspace

_CACHE: Dict[Tuple[str, Tuple[int, ...], int, int, str], Routine] = {}
_RE_ARX = re.compile(r"ARX\((\d+)\)\+EGARCH\((\d+),(\d+)\)\+(Normal|StudentT|SkewT|GED)$")
_RE_HARX = re.compile(r"HARX\(([0-9,]+)\)\+EGARCH\((\d+),(\d+)\)\+(Normal|StudentT|SkewT|GED)$")
...
def _parse_uid(uid: str) -> tuple[ARX | HARX, int, int, str]:
    ...
    raise ValueError(f"Unsupported uid for meanx_egarch: {uid}")


def _build(mean: ARX | HARX, p: int, q: int, density_name: str) -> Routine:
    ops = _DENSITY_OPS[density_name]
    uid = f"{mean.signature}+EGARCH({p},{q})+{density_name}"
    vol = EGARCH(p, q)
    dens = ops.factory()
    ...
    def fit(
        ...
        **_,
        ...
        def call_nll(theta: NDArray[np.float64]) -> float:
            ...
            return getattr(_core, f"_linear_mean_egarch_nll_pq_{ops.suffix}")(
        ...
        def call_grad(theta: NDArray[np.float64]) -> NDArray[np.float64]:
            ...
            if use_specialized:
                ...
                getattr(_core, f"_linear_mean_egarch_nll_grad_pq_{ops.suffix}")(
        ...
        def call_hess(theta: NDArray[np.float64]) -> NDArray[np.float64]:
            ...
            if use_specialized:
                ...
                getattr(_core, f"_linear_mean_egarch_hess_pq_{ops.suffix}")(
                    _as_cptr(theta), y_ptr, feat_ptr, resid_ptr, sigma2_ptr, hess_ptr, n, n_mean, p, q
                )
            return hess_mat.copy()

        mean_start = np.asarray(mean.default_start(y_arr), dtype=np.float64)
