...
from .transforms import (
    ...
    pack_arma_egarch_studentt,
    unpack_arma_egarch_studentt,
)

_CACHE: Dict[Tuple[int, int, int, int], Routine] = {}
_UID_RE = re.compile(r"^ARMA\((\d+),(\d+)\)\+EGARCH\((\d+),(\d+)\)\+StudentT$")
...
def _build(p_ar: int, q_ma: int, p_arch: int, q_egarch: int) -> Routine:
    uid = f"ARMA({p_ar},{q_ma})+EGARCH({p_arch},{q_egarch})+StudentT"
    mean = ARMA(p_ar, q_ma)
    ...
    def fit(
        ...
        **_,
        ...
        if special:
            grad_vec = np.zeros(8, dtype=np.float64)
            hess_mat = np.zeros((8, 8), dtype=np.float64)

            def call_nll(params: NDArray[np.float64]) -> float:
                return float(_core._arma_egarch_nll_11_studentt(_as_cptr(params), y_c, resid_c, sigma2_c, h0, n))

            def call_grad(params: NDArray[np.float64]) -> NDArray[np.float64]:
                _core._arma_egarch_nll_grad_11_studentt(
            ...
            def call_hess(params: NDArray[np.float64]) -> NDArray[np.float64]:
                _core._arma_egarch_hess_11_studentt(
            ...
            def call_nll(params: NDArray[np.float64]) -> float:
                return float(
                    _core._arma_egarch_nll_pq_studentt(
        ...
        if not log_mode:
            ...
            def obj_log(z: NDArray[np.float64]) -> float:
                return float(
                    _core._log_arma_egarch_nll_pq_studentt(
            ...
            def hess_log(z: NDArray[np.float64]) -> NDArray[np.float64]:
                theta_local = pack_arma_egarch_studentt(z, p_ar, q_ma, p_arch, q_egarch)
                grad_theta = call_grad(theta_local) * p_scaler
                hess_theta = call_hess(theta_local) * p_scaler
                return log_hessian_arma_egarch_studentt(
                    theta_local, grad_theta, hess_theta, p_ar, q_ma, p_arch, q_egarch
                )

            z0 = unpack_arma_egarch_studentt(theta0, p_ar, q_ma, p_arch, q_egarch)
            if solver.lower() == "nelder-mead":
            ...
            theta_hat = pack_arma_egarch_studentt(np.asarray(res.x, dtype=np.float64), p_ar, q_ma, p_arch, q_egarch)
            res.x = theta_hat.copy()
            res.fun = float(res.fun) / p_scaler
        ...
        if log_mode:
            hessian_theta, cov_matrix = compute_se_via_logspace(
                ...
                unpack_fn=lambda theta: unpack_arma_egarch_studentt(theta, p_ar, q_ma, p_arch, q_egarch),
                jacobian_fn=lambda theta: jacobian_arma_egarch_studentt(theta, p_ar, q_ma, p_arch, q_egarch),
                pack_fn=lambda z: pack_arma_egarch_studentt(z, p_ar, q_ma, p_arch, q_egarch),
                hess_z_fn=lambda z: log_hessian_arma_egarch_studentt(
                    pack_arma_egarch_studentt(z, p_ar, q_ma, p_arch, q_egarch),
                    call_grad(pack_arma_egarch_studentt(z, p_ar, q_ma, p_arch, q_egarch)),
                    call_hess(pack_arma_egarch_studentt(z, p_ar, q_ma, p_arch, q_egarch)),
                    p_ar,
                    q_ma,
                    p_arch,
                    q_egarch,
...
def get_routine(uid: str) -> Routine:
    ...
    if not match:
        raise RuntimeError(f"Unsupported UID '{uid}' for ARMA + EGARCH + StudentT.")
