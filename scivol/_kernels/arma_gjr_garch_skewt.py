"""
ARMA(p,q) + GJR-GARCH(P,Q) + SkewT kernel.
...
from .transforms import (
    ...
    pack_arma_gjr_garch_skewt,
    unpack_arma_gjr_garch_skewt,
)

_CACHE: Dict[Tuple[int, int, int, int], Routine] = {}
_RE_UID = re.compile(r"ARMA\((\d+),(\d+)\)\+GJR-GARCH\((\d+),(\d+)\)\+SkewT$")
...
def _build(p_ar: int, q_ma: int, P_arch: int, Q_garch: int) -> Routine:
    uid = f"ARMA({p_ar},{q_ma})+GJR-GARCH({P_arch},{Q_garch})+SkewT"
        ...
        **_,
        ...
        if use_specialized:
            ...
            def call_hess(params: NDArray[np.float64]) -> NDArray[np.float64]:
                _core._arma_gjr_garch_hess_11_skewt(
                    _as_cptr(params), y_ptr, resid_ptr, sigma2_ptr, hess_ptr, h0, n
                )
                return hess_mat.copy()
        else:
            def call_nll(params: NDArray[np.float64]) -> float:
                return _core._arma_gjr_garch_nll_pq_skewt(
                    _as_cptr(params),
                    y_ptr,
                    resid_ptr,
                    sigma2_ptr,
            ...
            def call_grad(params: NDArray[np.float64]) -> NDArray[np.float64]:
                _core._arma_gjr_garch_nll_grad_pq_skewt(
                    _as_cptr(params),
                    y_ptr,
                    resid_ptr,
                    sigma2_ptr,
            ...
            def call_hess(params: NDArray[np.float64]) -> NDArray[np.float64]:
                _core._arma_gjr_garch_hess_pq_skewt(
                    _as_cptr(params),
                    y_ptr,
                    resid_ptr,
                    sigma2_ptr,
        ...
        if not log_mode:
            ...
            def obj_log(z: NDArray[np.float64]) -> float:
                return _core._log_arma_gjr_garch_nll_pq_skewt(
                    _as_cptr(z),
                    y_ptr,
                    resid_ptr,
                    sigma2_ptr,
            ...
            def jac_log(z: NDArray[np.float64]) -> NDArray[np.float64]:
                _core._log_arma_gjr_garch_nll_grad_pq_skewt(
                    _as_cptr(z),
                    y_ptr,
                    resid_ptr,
                    sigma2_ptr,
            ...
            def analytical_hess_z(z: NDArray[np.float64]) -> NDArray[np.float64]:
                theta_local = pack_arma_gjr_garch_skewt(z, p_ar, q_ma, P_arch, Q_garch)
                grad_theta = call_grad(theta_local) * p_scaler
                hess_theta = call_hess(theta_local) * p_scaler
                return log_hessian_arma_gjr_garch_skewt(
                    theta_local,
                    grad_theta,
                    hess_theta,
                    p_ar,
            ...
            z0 = unpack_arma_gjr_garch_skewt(start, p_ar, q_ma, P_arch, Q_garch)
            ...
            theta_hat = pack_arma_gjr_garch_skewt(res.x, p_ar, q_ma, P_arch, Q_garch)
            res.x = theta_hat
        ...
        if log_mode:
            hessian, cov_matrix = compute_se_via_logspace(
                theta_hat=theta_hat,
                nll_theta=call_nll,
                unpack_fn=lambda th: unpack_arma_gjr_garch_skewt(th, p_ar, q_ma, P_arch, Q_garch),
                jacobian_fn=lambda th: jacobian_arma_gjr_garch_skewt(th, p_ar, q_ma, P_arch, Q_garch),
                pack_fn=lambda z: pack_arma_gjr_garch_skewt(z, p_ar, q_ma, P_arch, Q_garch),
...
def get_routine(uid: str) -> Routine:
    m = _RE_UID.match(uid)
    if not m:
        raise RuntimeError(f"arma_gjr_garch_skewt cannot handle '{uid}'")
