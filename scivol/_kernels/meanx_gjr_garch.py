...
from .linear_mean_gjr_garch_common import (
    _as_cptr,
    build_linear_mean_features,
    jacobian_linear_mean_gjr_garch_ged,
    jacobian_linear_mean_gjr_garch_normal,
    jacobian_linear_mean_gjr_garch_skewt,
    jacobian_linear_mean_gjr_garch_studentt,
    log_hessian_linear_mean_gjr_garch_ged,
    log_hessian_linear_mean_gjr_garch_normal,
    log_hessian_linear_mean_gjr_garch_skewt,
    log_hessian_linear_mean_gjr_garch_studentt,
    pack_linear_mean_gjr_garch_ged,
    pack_linear_mean_gjr_garch_normal,
    pack_linear_mean_gjr_garch_skewt,
    pack_linear_mean_gjr_garch_studentt,
    unpack_linear_mean_gjr_garch_ged,
    unpack_linear_mean_gjr_garch_normal,
    unpack_linear_mean_gjr_garch_skewt,
    unpack_linear_mean_gjr_garch_studentt,
)
from .routine import Routine
from .transforms import compute_se_via_logspace

_CACHE: Dict[Tuple[str, Tuple[int, ...], int, int, str], Routine] = {}
_RE_ARX = re.compile(r"ARX\((\d+)\)\+GJR-GARCH\((\d+),(\d+)\)\+(Normal|StudentT|SkewT|GED)$")
_RE_HARX = re.compile(r"HARX\(([0-9,]+)\)\+GJR-GARCH\((\d+),(\d+)\)\+(Normal|StudentT|SkewT|GED)$")
...
class _DensityOps:
    def __init__(
        ...
        signature: str,
        suffix: str,
        factory: Callable[[], Normal | StudentT | SkewT | GED],
...
def _parse_uid(uid: str) -> tuple[ARX | HARX, int, int, str]:
    arx = _RE_ARX.fullmatch(uid)
    if arx:
        lags, p, q = (int(arx.group(i)) for i in range(1, 4))
        return ARX(lags), p, q, arx.group(4)

    harx = _RE_HARX.fullmatch(uid)
    if harx:
        horizons = tuple(int(x) for x in harx.group(1).split(","))
        p = int(harx.group(2))
        q = int(harx.group(3))
        return HARX(horizons), p, q, harx.group(4)

    raise ValueError(f"Unsupported uid for meanx_gjr_garch: {uid}")


def _build(mean: ARX | HARX, p: int, q: int, density_name: str) -> Routine:
    ops = _DENSITY_OPS[density_name]
    uid = f"{mean.signature}+GJR-GARCH({p},{q})+{density_name}"
    vol = GJRGARCH(p, q)
    dens = ops.factory()
    ...
    def fit(
        ...
        **_,
        ...
        n_params = n_mean + n_vol + dens.n_params
        ...
        def call_nll(theta: NDArray[np.float64]) -> float:
            sigma2[0] = np.mean(y_arr * y_arr)
            if use_specialized:
                return getattr(_core, f"_linear_mean_gjr_garch_nll_11_{ops.suffix}")(
                    _as_cptr(theta), y_ptr, feat_ptr, resid_ptr, sigma2_ptr, n, n_mean
                )
            return getattr(_core, f"_linear_mean_gjr_garch_nll_pq_{ops.suffix}")(
                _as_cptr(theta), y_ptr, feat_ptr, resid_ptr, sigma2_ptr, n, n_mean, p, q
            )

        def call_grad(theta: NDArray[np.float64]) -> NDArray[np.float64]:
            sigma2[0] = np.mean(y_arr * y_arr)
            if use_specialized:
                getattr(_core, f"_linear_mean_gjr_garch_nll_grad_11_{ops.suffix}")(
                    _as_cptr(theta), y_ptr, feat_ptr, resid_ptr, sigma2_ptr, grad_ptr, n, n_mean
                )
            else:
                getattr(_core, f"_linear_mean_gjr_garch_nll_grad_pq_{ops.suffix}")(
                    _as_cptr(theta), y_ptr, feat_ptr, resid_ptr, sigma2_ptr, grad_ptr, n, n_mean, p, q
                )
        ...
        def call_hess(theta: NDArray[np.float64]) -> NDArray[np.float64]:
            sigma2[0] = np.mean(y_arr * y_arr)
            if use_specialized:
                getattr(_core, f"_linear_mean_gjr_garch_hess_11_{ops.suffix}")(
                    _as_cptr(theta), y_ptr, feat_ptr, resid_ptr, sigma2_ptr, hess_ptr, n, n_mean
                )
            else:
                getattr(_core, f"_linear_mean_gjr_garch_hess_pq_{ops.suffix}")(
                    _as_cptr(theta), y_ptr, feat_ptr, resid_ptr, sigma2_ptr, hess_ptr, n, n_mean, p, q
                )
            return hess_mat.copy()

        mean_start = mean.default_start(y_arr)
        vol_start = np.asarray(vol.default_start(y_arr), dtype=np.float64)
        dens_start = np.asarray(dens.default_start(y_arr), dtype=np.float64)
        start = np.concatenate([mean_start, vol_start, dens_start])

        bounds = list(mean.bounds()) + list(vol.bounds()) + list(dens.bounds())

        A = np.zeros((1, n_params), dtype=np.float64)
        A[0, n_mean + 1:n_mean + 1 + p] = 1.0
        A[0, n_mean + 1 + p:n_mean + 1 + 2 * p] = 1.0
        A[0, n_mean + 1 + 2 * p:n_mean + 1 + 2 * p + q] = 1.0
        ...
        if not log_mode:
            ...
            def pack(z: NDArray[np.float64]) -> NDArray[np.float64]:
                return ops.pack_fn(z, mean, p, q)

            def unpack(theta: NDArray[np.float64]) -> NDArray[np.float64]:
                return ops.unpack_fn(theta, mean, p, q)
            ...
            def jac_log(z: NDArray[np.float64]) -> NDArray[np.float64]:
                theta_local = pack(z)
                J = ops.jacobian_fn(theta_local, mean, p, q)
            ...
            def hess_log(z: NDArray[np.float64]) -> NDArray[np.float64]:
                ...
                return ops.log_hessian_fn(theta_local, grad_theta, hess_theta, mean, p, q)
        ...
        if log_mode:
            hessian_theta, cov_matrix = compute_se_via_logspace(
                ...
                unpack_fn=lambda theta: ops.unpack_fn(theta, mean, p, q),
                jacobian_fn=lambda theta: ops.jacobian_fn(theta, mean, p, q),
                pack_fn=lambda z: ops.pack_fn(z, mean, p, q),
                hess_z_fn=hess_log,
            )
        else:
    ...
    return Routine(uid=uid, fit=fit, n_params=mean.n_params + n_vol + dens.n_params)


def get_routine(uid: str) -> Routine:
    mean, p, q, density_name = _parse_uid(uid)
    key = (
    ...
    try:
        ...
        routine = _build(mean, p, q, density_name)
        _CACHE[key] = routine
        return routine
